...
int get_eth_ip(const char *eth_inf, char *ip); // 获取本机ip  
int get_local_ips(char *ip); // 获取本机ip列表
...
// 获取本机ip  
int get_eth_ip(const char *eth_inf, char *ip)  
{  
    int sd;  
...
int get_local_ip(char *ip)
{
	...
	for(i = 0; hent->h_addr_list[i]; i++) {
        if(strcmp("127.0.0.1", inet_ntoa(*(struct in_addr*)(hent->h_addr_list[i]))) != 0){
